"""
Main training script for Causal Autoencoder
...
from model_interface_causal import CausalModelInterface
from data_interface import DataInterface
from utils.logging import get_resume_info
from configs.config_schema_causal import load_config_with_schema, AppConfigCausal
...
def load_callbacks(cfg: AppConfigCausal):
    callbacks = []
...
def _build_trainer_kwargs(cfg: AppConfigCausal, logger: TensorBoardLogger, callbacks: list) -> Dict[str, Any]:
...
def main(cfg: AppConfigCausal, tracker: ConfigUsageTracker, runtime: Dict[str, Any]):
    ...
    if mode == 'warmstart' and ckpt_path:
        model_module = CausalModelInterface.load_from_checkpoint(
            ckpt_path,
        ...
        model_module = CausalModelInterface(**model_interface_kwargs)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config_path', default=os.path.join(os.getcwd(), 'configs', 'config.yaml'),
